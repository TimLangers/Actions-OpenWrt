...
# 3. 添加 OpenClash 源（使用 master 分支）
echo "src-git openclash https://github.com/vernesong/OpenClash.git;master" >> feeds.conf.default

# 4. 添加 Argon 主题及其配置插件（使用 master 分支确保兼容 25.12 架构）
echo "src-git luci_theme_argon https://github.com/jerrykuku/luci-theme-argon.git;master" >> feeds.conf.default
echo "src-git luci_app_argon_config https://github.com/jerrykuku/luci-app-argon-config.git;master" >> feeds.conf.default
